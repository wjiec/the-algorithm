 ...
 * and can move to an adjacent room. Moving between adjacent rooms takes one second for one move
        ...
        while (!pq.isEmpty()) {
            ...
            for (var dir : dirs) {
                ...
                if (dx >= 0 && dx < m && dy >= 0 && dy < n) {
                    long minCost = Math.max(moveTime[dx][dy] + (curr[2] % 2 + 1), dis[curr[0]][curr[1]] + (curr[2] % 2 + 1));
                    if (minCost < dis[dx][dy]) {
                        dis[dx][dy] = minCost;
                        pq.add(new int[]{dx, dy, curr[2] + 1});
                    }
                }
            }
        }
