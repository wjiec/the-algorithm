package offer.p67bazifuchuanzhuanhuanchengzhengshulcof;
...
public class Solution {
    ...
    public int strToInt(String str) {
        ...
        if (!Character.isDigit(chars[idx])) {
            switch (chars[idx]) {
                case '+' -> idx++;
                case '-' -> { negative = true; idx++; }
                default -> { return 0; }
            }
        }

        long ans = 0;
        for (; idx < n && Character.isDigit(chars[idx]); idx++) {
    ...
    public static void main(String[] args) {
        assert new Solution().strToInt("42") == 42;
        assert new Solution().strToInt("   -42") == -42;
        assert new Solution().strToInt("4193 with words") == 4193;
        assert new Solution().strToInt("words and 987") == 0;
        assert new Solution().strToInt("-91283472332") == -2147483648;
    }
