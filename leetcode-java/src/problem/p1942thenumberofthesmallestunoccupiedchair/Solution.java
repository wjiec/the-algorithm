package problem.p1942thenumberofthesmallestunoccupiedchair;

import java.util.Arrays;
import java.util.Comparator;
import java.util.PriorityQueue;
import java.util.TreeSet;

/**
 * 1942. The Number of the Smallest Unoccupied Chair
 *
 * https://leetcode.cn/problems/the-number-of-the-smallest-unoccupied-chair/
 *
 * There is a party where n friends numbered from 0 to n - 1 are attending.
 * There is an infinite number of chairs in this party that are numbered from 0 to infinity.
 * When a friend arrives at the party, they sit on the unoccupied chair with the smallest number.
 *
 * For example, if chairs 0, 1, and 5 are occupied when a friend comes, they will sit on chair number 2.
 * When a friend leaves the party, their chair becomes unoccupied at the moment they leave.
 * If another friend arrives at that same moment, they can sit in that chair.
 *
 * You are given a 0-indexed 2D integer array times where times[i] = [arrivali, leavingi], indicating
 * the arrival and leaving times of the ith friend respectively, and an integer targetFriend.
 *
 * All arrival times are distinct.
 *
 * Return the chair number that the friend numbered targetFriend will sit on.
 */

public class Solution {

    public int smallestChair(int[][] times, int targetFriend) {
        Integer[] index = new Integer[times.length];
        for (int i = 0; i < times.length; i++) index[i] = i;
        Arrays.sort(index, Comparator.comparingInt(i -> times[i][0]));

        int last = 0;
        TreeSet<Integer> returns = new TreeSet<>();
        // [time, chair]
        PriorityQueue<int[]> chairs = new PriorityQueue<>(Comparator.comparingInt(v -> v[0]));
        for (int i = 0; i < times.length; i++) {
            int idx = index[i];
            int arrival = times[idx][0], leave = times[idx][1];

            while (!chairs.isEmpty() && chairs.peek()[0] <= arrival) {
                returns.add(chairs.remove()[1]);
            }

            Integer curr = returns.pollFirst();
            if (curr == null) curr = last++;

            if (idx == targetFriend) return curr;
            chairs.add(new int[]{leave, curr});
        }
        return -1;
    }

    public static void main(String[] args) {
        assert new Solution().smallestChair(new int[][]{{1,4},{2,3},{4,6}}, 1) == 1;
        assert new Solution().smallestChair(new int[][]{{3,10},{1,5},{2,6}}, 0) == 2;
    }

}
